...
class MidonetApi(object):
    ...
    def add_bridge_dhcp(self, bridge, gateway_ip, cidr, host_rts=None,
        ...
        host routes, and dns name servers. This is currently used
        by the neutron plugin and the argument format is as follows:

        Keyword arguments
        gateway_ip -- single ipv4 address string.
        cidr -- subnet represented by cidr notation [ipv4 addr]/[subnet len]
        host_rts -- an array of dictionaries, each of the form:
            {"destination": <ipv4 cidr>, "nexthop": <ipv4 string>}
        dns_nservers -- an array of strings representing ipv4 addresses
